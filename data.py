...
def dataInput(retIx='cut'):
    ...
    # macro names
    n = ['rvq','drgdp','dip','dunemp','dhous','dcprof','infl',
            'nai','no','dconss','drcons','tspread']
    
    #MACRO VARIABLES
    df2 = pd.read_excel('data/cl_macrodata.xls',index_col=0,header=0)
    ix = pd.DatetimeIndex(df2.index.tolist())
    cols = ['RV','GDPD','IPI','UNEMP','HOUSE','PROFIT','INF','NAI','NOI',
                'CSI','CONS','SPREAD']
...
def descriptiveStats(ret,mvs,verbose=True):
    """Descriptive stats for the returns and the macroeconomic variables
        returns two latex tables of descriptive stats"""
    #Implementing the Descriptive Statistics
    if verbose: print("Data Analysis")
    cols = ['Obs.','Max','Mean','Median','Min','Std. Deviation','Skew','Kurtosis']
    dataAnalysis1 = pd.DataFrame(np.zeros((1,8)),index=['Returns'],columns=cols)
    ...
    if verbose: print(dataAnalysis1)
    ...
    if verbose: print(dataAnalysis2)
...
def recessionSE():
    ...
    if r.iloc[0,0] == 1: start.extend([r.index[0]])
    # add recession start and end dates
    for i in range(1,r.shape[0]):
        a = r.iloc[i-1,0]
        b = r.iloc[i,0]
        if a==0 and b==1: start.extend([r.index[i]])
        elif a==1 and b==0: end.extend([r.index[i-1]])
    # if there is a recession at the end, add the last date
    if len(start)>len(end): end.extend([r.index[-1]])
    return start, end
...
# Run the descriptive stats and time series IF this is the main script
if __name__ == "__main__":
    ...
    r_s,r_e = recessionSE()
    l = len(names)
    if np.remainder(l,4) == 1: unev,r=1,1
    elif np.remainder(l,4) == 2:unev,r=2,1
    elif np.remainder(l,4) == 3:unev,r=3,1
    else:unev,r=0,0
