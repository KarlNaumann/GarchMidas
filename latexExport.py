...
"""Starting with Tables.... tbc"""

import pandas as pd

def latex_matrix(df, eq=None, precision=None, bracket_cols=[], save_file=None, verbose=False):
    output = []
    if eq is not None: 
...
def latex_table(df,title,label,indexTitle='',save=True,precision=None,index=True,verbose=False):
    """df = desired dataframe, title = table title string, label = filename/label, save will export your file default true, 
    precision is the number of decimal places desired, index=True will take the """
    output=[]
            ...
            if type(df.index) == pd.core.indexes.multi.MultiIndex:
                for j in range(len(df.index[i])):
                    if i>0 and df.index[i][j]==df.index[i-1][j]:
                        newrow+='\\textbf{} & '
                    else: newrow+='\\textbf{%s} & '%df.index[i][j]
            else: newrow+='\\textbf{%s} & '%df.index[i]
        for j in range(cols):
            val = df.iloc[i,j]
            if precision is not None: val = round(val, precision)
            newrow+='{} & '.format(val) 
...
def regression_table(df_coef,df_se,title,label,df_pval=None,indexTitle='',save=True,precision=None,index=True,verbose=False):
    """df = desired dataframe, title = table title string, label = filename/label, save will export your file default true, 
    precision is the number of decimal places desired, index=True will take the """
    output=[]
    #Basic beginning structure of the LaTeX table
                ...
                if type(df_coef.index) == pd.core.indexes.multi.MultiIndex:
                    for j in range(len(df_coef.index[i])):
                        if i>0 and df_coef.index[i][j]==df_coef.index[i-1][j]:
                            newrow+='\\textbf{} & '
                        else: newrow+='\\textbf{%s} & '%df_coef.index[i][j]
                else: newrow+='\\textbf{%s} & '%df_coef.index[i]
            else:
                if type(df_coef.index) == pd.core.indexes.multi.MultiIndex:
